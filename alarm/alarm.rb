        ...
        if pkt.is_tcp?
    	    ...
    	    if flags.urg == 0 && flags.ack == 0 && flags.psh == 0 &&
    	       flags.rst == 0 && flags.syn == 0 && flags.fin == 0
                print_incident("NULL scan", pkt.ip_saddr, proto, payload)
    	    end
    	    if flags.urg == 1 && flags.ack == 1 && flags.psh == 1 &&
    	       flags.rst == 1 && flags.syn == 1 && flags.fin == 1 
                print_incident("XMAS scan", pkt.ip_saddr, proto, payload)
    	    end
            if pkt.payload[/4\d{3}(\s|-)?\d{4}(\s|-)?\d{4}(\s|-)?\d{4}/] ||
                ...
                print "ALERT: Credit card leaked in the clear from #{pkt.ip_saddr} (#{proto}) (#{payload})!"
            end
        end
    end
end
...
def check_line(line)
    ...
    if incident[:payload][/\\x[0-9, A-F][0-9, A-F]/] && incident[:payload][/^((?!HTTP).)*$/]
        ...
        print_incident("Shellcode", incident[:source], "TCP",
